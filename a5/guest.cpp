...
void guest(int id, int priority)
{
    ...
    {        
        ...
        if(curCleanRooms)
        {
            ...
            printf("Occupancy semaphore decremented to %d\n", curNetOcc-1);
            if(curNetOcc == 1)
            {
                ...
                printf("Cleaning started, waking up cleaner threads\n");
            ...
            // sleep for the allotted stay time
            sleep(guest.stay_time);
            printf("Guest [%d] finished stay in room [%d]\n", guest.id, r.room_id);
        ...
        auto it = hotel.nondirty_and_occupied_rooms.find(r);
        hotel.nondirty_and_occupied_rooms.erase(it);
        printf("Guest [%d] with %d priority allocated room [%d], replaces guest [%d] with %d priority\n", guest.id, guest.priority, r.room_id, r.guest.id, r.guest.priority);
        hotel.occupy(r, guest);
