...
void printDestinationPaths(const vector<int>& sources, vector<int>& dests, vector<vector<int>>& dist) 
{
    ...
    for (int i = 0; i < (int)dests.size(); i++) 
    {
        ...
        int u = dests[i];
        path.push_back(u);
        while (allSources.find(u) == allSources.end())
        {
            for (int v : adjList[u]) 
            {
        ...
        if(paths[dests[i]].size() == 0)
            paths[dests[i]] = path;
        else if(paths[dests[i]].size() > path.size())
            paths[dests[i]] = path;
    }
}

void multiSourceDijkstra(const vector<int>& sources) 
{
    set<int> sourcesSet(sources.begin(), sources.end());
    queue<int> q;
    ...
    printDestinationPaths(sources, dests, dist);
}
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        multiSourceDijkstra(vector<int> (newSources.begin(), newSources.end()));
